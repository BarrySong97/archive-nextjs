...
import { createRecord, getRecordList } from "@/db/service/recordService";
...
export const GET = withAuth(async (req: NextRequest, userId: string) => {
  const [err, records] = await to(getRecordList(userId));
  if (err) {
    return NextResponse.json({ error: "获取记录列表失败" }, { status: 500 });
  }
